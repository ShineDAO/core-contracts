...
const provider = new web3.providers.HttpProvider(
  process.env.MORALIS_AVALANCHE_TEST_KEY
);
  ...
  networks: {
    ...
    fuji: {
      provider: function () {
        return new HDWalletProvider({
          mnemonic: process.env.AVALANCHE_TEST_MNENONIC,
      ...
      //gas: web3.utils.toWei("0.5",'ether')
    },
    test: {
      host: "127.0.0.1", // Localhost (default: none)
      port: 8545, // Standard Ethereum port (default: none)
    ...
    ropsten: {
      provider: function () {
        return new HDWalletProvider(
          ...
          process.env.INFURA_ROPSTEN_API_KEY
        );
      },
      // Localhost (default: none)
    ...
    goerli: {
      provider: function () {
        return new HDWalletProvider(
          process.env.MNEMONIC,
          process.env.INFURA_GOERLI_API_KEY
        );
      },
      // Localhost (default: none)
    ...
    mainnet: {
      provider: function () {
        return new HDWalletProvider(
          process.env.MNEMONIC,
          process.env.INFURA_MAINNET_API_KEY
        );
      },
      // Localhost (default: none)
      network_id: "1", // Any network (default: none)
    ...
    polygon: {
      provider: function () {
        return new HDWalletProvider(
          process.env.MNEMONIC,
          process.env.INFURA_POLYGON_API_KEY
        );
      },
      // Localhost (default: none)
